...
| Window movement | Pointer move/resize | `smartmovewin`, `smartresizewin`, `movewin`, `resizewin` | pointer ops and resize primitives | `move-floating`, `resize-floating`, `pointer-bind ... move/resize` | X | Pointer bindings support floating windows and tiled drag/resize for built-in/native layouts, including drag floating/tiled toggle and tiled resize-edge double-click gestures. |
