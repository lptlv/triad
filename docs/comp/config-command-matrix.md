...
| Output | Native mirroring | `monitorrule ... mirror` | No stock River cloning protocol yet | Future `mirror` output rule | | Blocked until upstream River can render one logical output to multiple physical outputs, including cursor and layer-shell composition. A fullscreen screencopy client is presentation, not output cloning; Triad will not carry a River fork. |
