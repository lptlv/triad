...
| Output | Native mirroring | `monitorrule ... mirror` | No stock River cloning protocol yet | Reserved `mirror "SOURCE"` output rule | | **Blocked by stock River.** River must render one logical desktop to multiple physical heads and advertise a public cloning protocol before Triad can activate this rule. Screencopy, overlapping outputs, and DRM leases are not native cloning; see the [native mirroring roadmap](../todo.md#native-output-mirroring). Triad will not carry a River fork or require a private build. |
